...
namespace UnityEditor.PostProcessing
{
    public sealed class CurveEditor
    {
        ...
        public void Add(SerializedProperty curve, CurveState state)
        {
            ...
            AnimationCurve animCurve = curve.animationCurveValue;
            if (animCurve == null)
            {
        ...
        public Selection GetSelection()
        {
            ...
            {
                AnimationCurve curve = m_SelectedCurve.animationCurveValue;
        ...
        public bool OnGUI(Rect rect)
        {
            ...
            GUI.BeginClip(rect);
            {
                ...
                foreach (KeyValuePair<SerializedProperty, CurveState> curve in m_Curves)
                {
        ...
        private void OnCurveGUI(Rect rect, SerializedProperty curve, CurveState state)
        {
            ...
            if (length == 0)
            {
                Vector3 p1 = CurveToCanvas(new Vector3(bounds.xMin, state.zeroKeyConstantValue));
                Vector3 p2 = CurveToCanvas(new Vector3(bounds.xMax, state.zeroKeyConstantValue));
                Handles.DrawAAPolyLine(state.width, p1, p2);
            }
            else if (length == 1)
            {
                Vector3 p1 = CurveToCanvas(new Vector3(bounds.xMin, keys[0].value));
                Vector3 p2 = CurveToCanvas(new Vector3(bounds.xMax, keys[0].value));
                Handles.DrawAAPolyLine(state.width, p1, p2);
            }
            else
            {
                Keyframe prevKey = keys[0];
                for (int k = 1; k < length; k++)
                {
                    Keyframe key = keys[k];
                    Vector3[] pts = BezierSegment(prevKey, key);

                    if (float.IsInfinity(prevKey.outTangent) || float.IsInfinity(key.inTangent))
                    {
                        Vector3[] s = HardSegment(prevKey, key);
                ...
                if (keys[0].time > bounds.xMin)
                {
                    ...
                    {
                        Vector3 p1 = CurveToCanvas(new Vector3(bounds.xMin, keys[0].value));
                        Vector3 p2 = CurveToCanvas(keys[0]);
                        Handles.DrawAAPolyLine(state.width, p1, p2);
                    }
                }
                ...
                if (keys[length - 1].time < bounds.xMax)
                {
                    ...
                    {
                        Vector3 p1 = CurveToCanvas(keys[length - 1]);
                        Vector3 p2 = CurveToCanvas(new Vector3(bounds.xMax, keys[length - 1].value));
                        Handles.DrawAAPolyLine(state.width, p1, p2);
                    }
                }
            }
            ...
            for (int k = 0; k < length; k++)
            {
                ...
                Vector3 pos = CurveToCanvas(keys[k]);
                Rect hitRect = new Rect(pos.x - 8f, pos.y - 8f, 16f, 16f);
                RectOffset offset = isCurrentlySelectedCurve
                    ? new RectOffset(5, 5, 5, 5)
                    : new RectOffset(6, 6, 6, 6);

                Vector3 outTangent = pos + CurveTangentToCanvas(keys[k].outTangent).normalized * 40f;
                Vector3 inTangent = pos - CurveTangentToCanvas(keys[k].inTangent).normalized * 40f;
                Rect inTangentHitRect = new Rect(inTangent.x - 7f, inTangent.y - 7f, 14f, 14f);
                Rect outTangentHitrect = new Rect(outTangent.x - 7f, outTangent.y - 7f, 14f, 14f);
                ...
                if (state.showNonEditableHandles)
                {
                    ...
                    {
                        Color selectedColor = (isCurrentlySelectedCurve && isCurrentlySelectedKeyframe)
                    ...
                    if (e.type == EventType.MouseDown && rect.Contains(e.mousePosition))
                    {
                        if (hitRect.Contains(e.mousePosition))
                        {
                            ...
                            else if (e.button == 1)
                            {
                                ...
                                {
                                    MenuAction action = (MenuAction)x;
                                    AnimationCurve curveValue = action.curve.animationCurveValue;
        ...
        private void OnGeneralUI(Rect rect)
        {
            ...
            if (e.type == EventType.MouseDown)
            {
                ...
                Vector3 hit = CanvasToCurve(e.mousePosition);
                ...
                foreach (KeyValuePair<SerializedProperty, CurveState> curve in m_Curves)
                {
                    ...
                    SerializedProperty prop = curve.Key;
                    CurveState state = curve.Value;
                    AnimationCurve animCurve = prop.animationCurveValue;
                    float hitY = animCurve.length == 0
                        ? state.zeroKeyConstantValue
                        : animCurve.Evaluate(hit.x);

                    Vector3 curvePos = CurveToCanvas(new Vector3(hit.x, hitY));
            ...
            if (e.type == EventType.KeyDown && (e.keyCode == KeyCode.Delete || e.keyCode == KeyCode.Backspace))
            {
                ...
                {
                    AnimationCurve animCurve = m_SelectedCurve.animationCurveValue;
        ...
        {
            Rect bounds = settings.bounds;
            Vector3 output = new Vector3((position.x - bounds.x) / (bounds.xMax - bounds.x), (position.y - bounds.y) / (bounds.yMax - bounds.y));
